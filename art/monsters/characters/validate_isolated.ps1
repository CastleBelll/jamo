...
$repoRoot = Split-Path (Split-Path (Split-Path $assetRoot -Parent) -Parent) -Parent
$validationRoot = Join-Path ([IO.Path]::GetTempPath()) ('JAMO_ASSET03_' + [guid]::NewGuid().ToString('N'))
...
Copy-Item -LiteralPath "$assetRoot/asset_stats.json", "$assetRoot/verify_characters.gd" -Destination "$validationRoot/art/monsters"
...
& $Godot --headless --path $validationRoot --script art/monsters/verify_characters.gd *> "$assetRoot/godot_isolated_verify.log"
$verifyExit = $LASTEXITCODE
$errors = Select-String -Path "$assetRoot/godot_isolated_import.log", "$assetRoot/godot_isolated_verify.log" -Pattern 'ERROR:|SCRIPT ERROR|CHARACTERS_FAIL'
