...
## Boss figures: their own model file, 1.2x the enemy height (fits between the HUD band and the
## B11 anchor at y=260 without clipping), no gait.
func spawn_boss_proxy(boss_id: StringName) -> CharacterProxy:
	return spawn_scene(CharacterProxy.model_for_boss(boss_id), &"GLIDE", CharacterProxy.TARGET_HEIGHT * 1.2)
