...
- **감각 패스(2026-09-13)**: 사용자 피드백(선이 거슬림·정적임·손패가 남의 자모 같음)에 따라 버튼/타일은 평면 먹·종이 스타일로 복귀(붓 빗살 제거), 전투 화면 가장자리 먹 번짐 제거·레인 선 약화, `Juice` 오토로드(버튼 hover/press 스케일, 로고 호흡, 메뉴 슬라이드 인, 자모 스폰 팝), Forge에 "내 덱 N장을 섞어 7장을 뽑음 · 남은 활자" 표시.

## 5b. P6 입체 캐릭터 (2026-09-13)
...
- **가독성 패스(2026-09-14)**: 사용자 지시 "최대한 글을 줄이고 딱 봐도 이해할 수 있게". p18 `400f5fa`(메뉴 아이콘 실제 실루엣, 연구 카드 = 아이콘·이름·숫자 한 줄·가격·버튼), p19 `85faa23`(사전 타일 = 아이콘+이름만·미발견 흐림·잠김 자물쇠, 상세 카드 = 아이콘·자모 글리프·랭크별 한 줄·핍·보스 글리프, 기록 배지 아이콘 3개, 설정 요약줄·안내문 삭제·슬라이더 숫자, 좌하단 통계 = HUD 아이콘+숫자), p20 `d0cf01c`(Forge "신규"·안내문 제거, Reroll ×N, 클리어 통계는 0이 아닌 값만, 결과 화면 빈 줄 생략). 각각 Opus QA PASS. p21 `9b53a0e`: 텍스트 크기 설정이 실제로 무효였던 버그(테마가 타입별 크기를 고정) 수정 — 프로젝트 테마의 모든 font_size 항목과 노드별 오버라이드를 기준값 대비 비율로 스케일, 코드 생성 라벨은 `SettingsService.px()`, BannerLabel/TitleLabel·글리프 타일·HUD는 고정. 남은 MINOR: 150%에서 Wave Clear 배너가 카드에 가려짐, RUN 중 일시정지에서 배율 변경 시 보상 타일은 다음 Wave부터 반영.
- **예정(사용자 지시)**: codex 사용량 리셋(2026-09-14 14:00) 후 Blender MCP 실연결로 자모/보스 24종을 다시 생성해 현재 bpy 모델을 교체. 파일명·클립명(idle/walk/hit/purify)·규격은 `art/models/MODELS.md` 그대로 유지하면 코드 변경 없이 교체됨.
...
2. **음향**: 코드 측은 연결됨 — `Sfx.set_boss_layer`(보스 Wave에서 `bgm_boss_layer.ogg`를 전투 루프 위치에 맞춰 1초 페이드), UI 버스 `play_ui`(모든 BaseButton 진입 시 자동 연결: Primary=확정음, Back/Abandon/Close/Skip=취소음, 나머지 클릭음, hover 톡), 보스 정화음. 파일(`bgm_boss_layer.ogg`, `ui_*.ogg`, `boss_purified.ogg`) 전부 반영됨.
...
6. ~~음향 후속~~ 완료(2026-09-12 밤): `bgm_boss_layer.ogg`(전투 루프와 16.00초 동기), `ui_click/confirm/cancel/hover.ogg`, `boss_purified.ogg`가 codex/audio로 합류, `Sfx`가 일시정지 중에도 동작하도록 `PROCESS_MODE_ALWAYS`.
