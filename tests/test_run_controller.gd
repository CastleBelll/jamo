...
func _check_run_game_scene() -> void:
	...
	_expect(hud.get_node("%StabilityLabel").text.begins_with("100/"), "HUD stability label")
	...
	_expect(hud.get_node("%StabilityLabel").text.begins_with("92/"), "HUD follows stability")
	run.on_wave_cleared()
