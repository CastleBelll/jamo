...
func _check_screen() -> void:
	...
	add_child(lib)
	_expect(not lib.get_node("%Opening").visible and lib.get_node("%GoldLabel").text == "50", "hub after the first run")
	...
	_expect(lib.get_node("%CodexRows").get_child_count() == 4 and lib.get_node("%CodexDetail").get_node("Head/Text/Title").text.begins_with("거대한 ㅁ (W5)"), "4 bosses, first detail")
	lib._open_setup()
	...
	add_child(lib)
	lib._set_codex_tab("compound")
	_expect(lib.get_node("%CodexDetail").get_node("Meta").get_child(0).text.begins_with("첫 합성"), "compound codex shows a discovery record, no 복원도")
	lib.free()
	Meta.events = ["S_SILENCE"]
