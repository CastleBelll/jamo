...
func _check_settings() -> void:
	...
	root.theme.set_font_size("font_size", "Label", 30)
	SettingsService.apply_text_scale(root, 150)
	_expect(root.theme.default_font_size == 42 and root.theme.get_font_size("font_size", "Label") == 45, "150%% text scale = 42px default, 45px Label")
	SettingsService.apply_text_scale(root, 125)
	_expect(root.theme.get_font_size("font_size", "Label") == 38, "125%% scales from the base, not from the previous scale")
	SettingsService.apply_text_scale(root, 100)
	_expect(root.theme.get_font_size("font_size", "Label") == 30, "100%% restores the base size")
	var child := Label.new()
	child.add_theme_font_size_override("font_size", 24)
	root.add_child(child)
	var banner := Label.new()
	banner.theme_type_variation = &"BannerLabel"
	banner.add_theme_font_size_override("font_size", 128)
	root.add_child(banner)
	SettingsService.apply_text_scale(root, 150)
	_expect(child.get_theme_font_size("font_size") == 36 and banner.get_theme_font_size("font_size") == 128, "per-node override scales to 36, banner stays 128")
	SettingsService.apply_text_scale(root, 100)
	_expect(child.get_theme_font_size("font_size") == 24, "override returns to its base")
	var project_root := Control.new()
	SettingsService.apply_text_scale(project_root, 150)
	_expect(ThemeDB.get_project_theme().get_font_size("font_size", "Label") == 45, "no node theme: the project theme scales")
	SettingsService.set_and_save("text_scale", 150)
	_expect(SettingsService.px(24) == 36, "px() follows the saved scale")
	SettingsService.set_and_save("text_scale", 100)
	SettingsService.apply_text_scale(null, 100)
	_expect(ThemeDB.get_project_theme().get_font_size("font_size", "Label") == 30, "null root resets the shared theme")
	project_root.free()
	root.free()
	Meta.new_profile()
