...
func _ready() -> void:
	...
	Meta.saver.path = "user://test_boss.json"
	Meta.new_profile()
...
func _check_result_and_retry() -> void:
	...
	_expect("W5 도달 · W4 클리어" in text, "reached vs cleared wave (%s)" % text)
	_expect("도달 70 · 패턴 30" in text, "top loss causes ordered (%s)" % text)
	_expect("빌드 · 검 R1" in text and "신규 · 검" in text and "12G" in text, "build, discoveries, integer gold")
	_expect("다음 · 칼: ㅋ 회수" in text, "one next goal from the pin (%s)" % text)
